...
class DatasetClassSampler:
    ...
    def sample_one(self, class_name: str, candidate_indices: Sequence[int] | None = None) -> tuple[int, pd.Series]:
    ...
    def sample_many(self, class_name: str, count: int) -> list[pd.Series]:
        ...
        indices = (
            ...
            if replace else self._rng.sample(candidates.index.tolist(), count)
        )
    ...
    def _class_candidates(self, class_name: str, candidate_indices: Sequence[int] | None = None) -> pd.DataFrame:
