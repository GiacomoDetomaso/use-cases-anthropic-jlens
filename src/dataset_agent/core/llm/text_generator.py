...
class SupportsStructuredOutput(Protocol):
    """Chat model interface required by structured generation helpers."""

    def with_structured_output(self, schema: type[BaseModel], *, include_raw: bool) -> Any: ...
...
def _is_context_length_error(error: BadRequestError) -> bool:
    ...
    return (
        ...
        and "input_tokens" in error_message
    )


def _build_generation_messages(
    source: InputDatasetModel, target: InputAttackModel
) -> list[BaseMessage]:
...
def _handle_generation_error(
    ...
    if isinstance(error, BadRequestError) and _is_context_length_error(error):
        ...
        raise FailedGenerationError(retries=retries) from error
...
def generate_messages_sync(
    chat_model: SupportsStructuredOutput,
    base_messages: list[BaseMessage],
    output_schema: type[BaseModel],
) -> BaseModel:
    ...
    while retries < _INTERNAL_RETRIES:
        try:
            ...
            response = llm.invoke(_build_input_messages(base_messages, retry_instruction))
    ...
    raise FailedGenerationError(retries=retries)
...
async def generate_messages_async(
    ...
    while retries < _INTERNAL_RETRIES:
        try:
            ...
            response = await llm.ainvoke(_build_input_messages(base_messages, retry_instruction))
