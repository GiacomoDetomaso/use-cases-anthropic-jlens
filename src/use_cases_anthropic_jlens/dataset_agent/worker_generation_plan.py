...
from use_cases_anthropic_jlens.dataset_agent.dataset_source import get_source_dataset
...
def _source_class_count(source_dataset: pd.DataFrame) -> dict[str, int]:
    label_column = settings.source_dataset.label_col_name
    return cast(dict[str, int], source_dataset[label_column].value_counts().to_dict())
