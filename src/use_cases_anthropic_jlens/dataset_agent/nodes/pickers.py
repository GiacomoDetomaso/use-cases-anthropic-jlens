...
from use_cases_anthropic_jlens.dataset_agent.core.instance_pickers.balancer import (
    DistributionBalancer,
)
from use_cases_anthropic_jlens.dataset_agent.core.instance_pickers.sampler import (
    DatasetClassSampler,
)
...
from use_cases_anthropic_jlens.settings import DatasetSettings, settings
...
class PickerInputDatasetNode:
    def __init__(
        self, dataset: pd.DataFrame, dataset_settings: DatasetSettings, seed: int | None = None
    ):
        ...
        self._dataset_class_count = cast(
            dict[str, int],
            dataset[dataset_settings.label_col_name].value_counts().to_dict()
        )
        ...
        return state.model_copy(
            update={
                ...
                "remaining_input_indices": [
                    index for index in state.remaining_input_indices if index != selected_index
                ],
                "input_distribution": self.balancer.update_class_distribution(
                    input_distribution, class_name
                ),
            }
        )
...
class PickerTargetDatasetNode:
    def __init__(
        ...
        self.dataset_settings = dataset_settings
        self.balancer = DistributionBalancer.from_settings(
            dataset_settings, target_size=target_size, seed=seed
        )
        self.sampler = DatasetClassSampler(
            dataset=dataset,
            data_col_name=dataset_settings.data_col_name,
    ...
    def __call__(self, state: DatasetState) -> DatasetState:
        ...
        return state.model_copy(
            update={
                ...
                "target_distribution": self.balancer.update_class_distribution(
