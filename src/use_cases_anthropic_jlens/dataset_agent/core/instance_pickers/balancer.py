...
from use_cases_anthropic_jlens.dataset_agent.core.instance_pickers.distribution_calculator import (
...
if TYPE_CHECKING:
    from use_cases_anthropic_jlens.settings import DatasetSettings
...
class DistributionBalancer:
    ...
    def update_class_distribution(
        self, distribution: DistributionState, class_name: str, amount: int = 1
    ) -> DistributionState:
        ...
        updated_distribution[class_name] = bucket.model_copy(
            update={"actual": bucket.actual + amount}
        )
